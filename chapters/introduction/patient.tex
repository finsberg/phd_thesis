...
\begin{figure}[htbp]
  \centering
  \begin{subfigure}[t]{0.3\textwidth}
    \includegraphics[width=\textwidth]{chapters/introduction/figures/geometry/strain_mesh_plane}
...
imaging data is not the stress-free geometry, meaning that the geometry
...
An alternative to the backward displacement method that does not
...
volume was less than $5 \%$. This method is general and could be applied
...
%   \State Initialize $\mathbf{m}_0$ \Comment{intial guess for material parameters}
%   \State $i = 0$
    
%   \While{difference in reference cavity volumes $>5 \%$ and $i < 15$}
...
% Note that this method apply also to bi-ventricular geometries and to
...
Another observation that is encountered in this thesis is strain, or
more precisely average regional strain in the 
...
Note that a solution to \eqref{eq:pde_unconstrained} satisfies the optimality
...
parameter space to exclude such solutions might help to stabilize the
Newton solver. 
...
your 
model is rich enough to represent the data, and the identifiability
...
\subsection{Extraction of biomarkers}
...
% Extracting biomarkers from computational models can be used
% to gain insight into physiological processes by e.g see how
% physiology changes when parameters or input are perturbed
% \cite{remme2011mechanics}. A model allows you to test 


% which potentially can be used to gain further
% insight into 
