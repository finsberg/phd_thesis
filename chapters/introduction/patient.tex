...
medical image, which is loaded with some loads $\mathbf{T}$. We want
to find the unloaded geometry 
$\Omega_{\mathcal{U}}$, so that when we load $\Omega_{\mathcal{U}}$ with $\mathbf{T}$,
...
of \eqref{eq:intro_variational_form} with $\mathbf{T}$ representing
...
when dealing with physical systems such as the heart or weather
forecast, it is not always possible to design an experiment which
...
that is taken in this thesis. For a more thorough review of other
...
points in the cardiac cycle, measured using e.g ultrasound and image
...
number of iterations. Such a minimum may not always exists, or there
may be several local minima. In the latter case, the choice of initial
...
\begin{remark}
  ...
  your measurements. In the latter case, one compute the strain tensor
...
\cite{UFL} in FEniCS to derive the discrete adjoint equations from
the discrete forward equations, and then the FEniCS system derives the
...
complexity of model in terms of \emph{model fidelity}, i.e whether your
...
encountered in other scientific disciplines, for instance in
...
Para maters in the model are estimated based on minimizing a cost
functional representing the mismatch between observations and simulating.
