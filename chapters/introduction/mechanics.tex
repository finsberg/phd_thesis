...
Since 2-4\% is very little, a common assumption in cardiac modeling,
which has also been made in the work conducted in this thesis, is to assume
the myocardium to be incompresible. The reason for this choice is
purely numerical.
...
\end{align}
However, the pressure $p_{\mathrm{lv}}$ is usually set
to zero.
...
\begin{remark}
  ...
  issue is to 1) use other types of elements that are continusly
...
active strain formaulation properties such as frame invariance and
rank-one ellipticity is inherited from the strain energy function. In
contrast, rank-one elipticity is not guaranteed for the active stress
formulation.  
